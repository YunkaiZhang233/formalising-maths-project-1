...
def fact_to_over_of_under : Factorisation f ⥤ Over (Under.mk f) where
...
def over_of_under_to_fact : Over (Under.mk f) ⥤ Factorisation f where
...
def factEqUnderOfOver : Factorisation f ≌ Over (Under.mk f) where
  functor := fact_to_over_of_under f
  inverse := over_of_under_to_fact f
